...
- cd turniermanager
- cp .env.example .env
### Passe DB_CREDENTIALS in .env an (deine MariaDB-Daten)
...
- **Admin Login**: Erstelle ersten Admin-User über `/api/auth/register`
...
### Auth
- POST /api/auth/register { "name": "Max", "email": "max@test.com", "password": "pass123" }
- POST /api/auth/login { "email": "max@test.com", "password": "pass123" } → { "token", "user" }
- GET /api/auth/me → JWT required

### Tournaments (Admin-only CRUD)
- GET /api/tournaments
- GET /api/tournaments/:id
- POST /api/tournaments { "title": "WM 2025", "mode": "KO", "max_players": 16 }
- POST /api/tournaments/:id/register
- POST /api/tournaments/:id/generate-bracket

### Matches
- GET /api/tournaments/:id/matches
- POST /api/matches/:id/result { "score_a": 3, "score_b": 1 }
...
**MariaDB Tabellen**: `users`, `tournaments`, `registrations`, `matches`, `messages`

-- Vollständiges Schema wird via Sequelize Migrationen erstellt
...
**Apache2 Features**: mod_proxy, mod_proxy_wstunnel, .htaccess für React Router
...
**Autor**: [DS-Coding]  
**Email**: admin@ds-coding.de  
**Demo**: http://localhost (nach `docker-compose up`)  
**Issues**: [Neues Issue](https://github.com/yourusername/turniermanager/issues/new)
